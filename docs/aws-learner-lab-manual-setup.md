...
| Upload bucket | `csv-insight-upload-s4143180-701711767320` |
| Avatar bucket | `csv-insight-avatar-s4143180-701711767320` |
...
- Bucket name: `csv-insight-upload-s4143180-701711767320`
...
```text
csv-insight-avatar-s4143180-701711767320
```
...
```text
backend/functions/upload_event/handler.py
```
...
Copy-Item `
  backend\api\app `
  backend\api\.lambda-package\app `
  -Recurse `
  -Force

Copy-Item `
  backend\api\lambda_handler.py `
  backend\api\.lambda-package\lambda_handler.py `
  -Force

Compress-Archive `
  -Path backend\api\.lambda-package\* `
  -DestinationPath backend\api\flask-api.zip `
  -Force
```
...
3. Upload `backend/api/flask-api.zip`.
4. Chọn **Save**.
...
1. Xóa và build lại `backend/api/.lambda-package` nếu dependency thay đổi.
2. Tạo lại `backend/api/flask-api.zip`.
...
- [ ] Learner Lab đang chạy.
- [ ] AWS CLI profile `learner-lab` hoạt động.
- [ ] Docker Desktop đang chạy.
- [ ] `CsvInsightUsers` và `email-index` ở trạng thái Active.
- [ ] `CsvInsightDatasets` ở trạng thái Active.
- [ ] Upload bucket và avatar bucket đã có CORS.
- [ ] Processor image đã được push lên ECR.
- [ ] ECS cluster và task definition đã được tạo.
- [ ] ECS security group cho phép outbound traffic.
