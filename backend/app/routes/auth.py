...
def login():
    ...
    try:
        password_hasher.verify(user["passwordHash"], password)
    except VerifyMismatchError as error:
        raise ApiError("Email or password is incorrect.", 401, "INVALID_CREDENTIALS") from error
...
def reset_password():
    ...
    store.update_user(
        user["userId"],
        {
            "passwordHash": password_hasher.hash(new_password),
            "resetTokenHash": "",
            "resetTokenExpiresAt": "",
            "updatedAt": datetime.now(timezone.utc).isoformat(),
        },
